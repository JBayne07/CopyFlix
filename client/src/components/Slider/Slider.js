...
export const Slider = ({ movies }) => {
  const [index, setIndex] = useState(0); // integer to determine the middle item
  const [numItems, setNumItems] = useState(5); // integer to determine how many items to show to the user
  const [style, setStyle] = useState({}); // object to determine how much to move the slider to the right or left
  const [hasSliderMoved, sethasSliderMoved] = useState(false); // boolean to display prev arrow
    ...
    const handleSliderAnimation = () => {
      if (sliderMoving) {
        ...
        if (sliderMoveDirection === "right") {
          translate = `translateX(-${hasSliderMoved ? 200 + 100 / numItems : 100}%)`;
        ...
        setStyle({
          transform: `translateX(-${(hasSliderMoved ? 100 + 100 / numItems : 0)}%)`
        });
      }
    }
  ...
  const handleItems = () => {
    const leftIndex = index - numItems < 0 && hasSliderMoved ? movies.length - numItems : index - numItems;
    const midIndex = index;
    const rightIndex = index + numItems > movies.length-1 ? 0 : index + numItems;
    const leftMostIndex = leftIndex - 1 < 0 ? movies.length-1 : leftIndex-1;
    const rightMostIndex = rightIndex+numItems + 1 > movies.length-1 ? 0 : rightIndex+numItems; 
    ...
    while(i !== rightIndex+numItems){
      ...
      if(movies[i]){
        output.push(<SliderItem movie={movies[i]} key={movies[i].Id} width={100/numItems}/>);
      }
      i++;
    }

    if(movies[leftMostIndex] && hasSliderMoved){
      output.unshift(<SliderItem movie={movies[leftMostIndex]} key={movies[leftMostIndex].Id} width={100/numItems} />);
    }
    if(movies[rightMostIndex]){
      output.push(<SliderItem movie={movies[rightMostIndex]} key={movies[rightMostIndex].Id} width={100/numItems} />)
    }
    return output;
  };
